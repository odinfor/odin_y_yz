...
class RunTestCase():
    ...
    def runtest(self):
        ...
        # test = getinputdata(type='updateprocess', case='code_is_null', **inputdata)     # 修改流程接口code为空测试场景
        test = getinputdata(type='updateprocess', case='nodeSetting_is_null', **inputdata)      # 修改流程接口nodeSetting为空测试场景
        interfacetype = test[0]     # 流程接口类型
        interfacedata = test[1]     # 流程接口传入参数

        log.debug('*' * 30 + '这是分割线开始,当前配置环境:{}'.format(operating_environment) + '*' * 30)
        # 列表新
